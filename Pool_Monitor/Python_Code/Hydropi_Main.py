...
# within the limits.
# If you change the state of a relay manually there may be a delay in it being 
# actioned if the sensors are being read at the same time, as it takes about 2s 
# to read each sensor. Also the pause delay start time is only set when the
# program attempts to read the sensors so the pause in reading with the default
# setting may be upto 35 min.
#
...
alert_check = False
sensor_ref_time = datetime.datetime.now()  # Set starting time
email_sent = False
email_sent_reset = 0
new_pause = True
...
try:
    while True:  # Repeat the code indefinitely
        ...
        if datetime.datetime.now() >= sensor_ref_time + datetime.timedelta(seconds=time_between_readings):
            sensor_ref_time = datetime.datetime.now()
            ...
            if delays["pause_readings"] == 0:
                ...
                if alert_check is True and email_sent is True:
                    ...
                    if alert_check is True:
                        send_email(alert_readings)
                        email_sent = True
                        email_sent_time = datetime.datetime.now()
            
            elif delays["pause_readings"] == 1:
                if new_pause is True:
                    pause_start_time = datetime.datetime.now()
                    new_pause = False
                if datetime.datetime.now() >= pause_start_time + datetime.timedelta(seconds=pause_time):
                    reset_pause_readings()
                    new_pause = True

            if email_sent is True:
                if datetime.datetime.now() >= email_sent_time + datetime.timedelta(seconds=email_time_delay):
                    alert_check = False
                    email_sent_time = datetime.datetime.now()
